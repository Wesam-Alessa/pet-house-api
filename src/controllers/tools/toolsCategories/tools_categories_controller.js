...
const {ToolsCategory} = require('../../../models/tools/tools_category');
const {User} = require('../../../models/users/user');

module.exports = {
    add_tools_category: async (req,res)=>{
        ...
        cloudinary.uploader.upload_stream({ folder: 'pet_house_server/tools/categories' }, (error, result) => {
        if (error) {
          console.error(error);
    ...
    update_tools_category:async (req,res)=>{
        if(req.header("x-auth-token") == undefined || req.header('uid') == undefined ||
                 req.header("x-auth-token") == "" || req.header('uid') == "" ) {
          return res.status(401).send("ERROR : some data error, please enter correct data ! ");
        }   
        ...
        let category = await ToolsCategory.findById(req.header('cid'));
        if(!category){
    ...
    get_tools_categories:async (req,res)=>{
        if(req.header("x-auth-token") == undefined || req.header('uid') == undefined ||
          req.header("x-auth-token") == "" || req.header('uid') == "" ) {
          return res.status(401).send("ERROR : some data error, please enter correct data ! ");
        }   
        ...
        let categories = await ToolsCategory.find();
        if(user.isAdmin == false){
